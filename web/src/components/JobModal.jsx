  ...
  return (
    <div className="card" style={{ padding: 12 }}>
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: singleDate ? "1fr" : "1fr 1fr",
          gap: 12,
        }}
      >
        <div>
...
const FlatHourlyBlock = memo(function FlatHourlyBlock({
  flatRate,
  setFlatRate,
  flatOT,
  setFlatOT,
...
export default function JobModal({ open, job, onClose, onCreated, onUpdated }) {
  ...
  function buildTierRates() {
    ...
    const priceFor = (tier) => {
      if (kind === "half_day")
        return tier === "jr" ? N(pHalfJr) : tier === "sr" ? N(pHalfSr) : N(pHalfLead);
      if (kind === "full_day")
        return tier === "jr" ? N(pFullJr) : tier === "sr" ? N(pFullSr) : N(pFullLead);
      if (kind === "2d1n")
        return tier === "jr" ? N(p2d1nJr) : tier === "sr" ? N(p2d1nSr) : N(p2d1nLead);
      return /* 3d2n */ tier === "jr"
        ? N(p3d2nJr)
        : tier === "sr"
        ? N(p3d2nSr)
