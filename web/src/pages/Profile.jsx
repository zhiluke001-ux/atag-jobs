...
function Row({ label, value, onEdit }) {
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, color: "#111", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {value || "—"}
      </div>
...
export default function Profile() {
  ...
  async function onPickAvatar(e) {
    ...
    if (!okTypes.includes(file.type)) { showToast("Only PNG/JPG/WEBP", "err"); return; }
  ...
  async function savePassword() {
    if (pwd.next !== pwd.confirm) { showToast("Passwords do not match", "err"); return; }
    if ((pwd.next || "").length < 6) { showToast("Password too short (min 6)", "err"); return; }
    setBusy(true);
    try {
  ...
  const avatarSrc =
    me?.avatarUrl ||
    "https://api.dicebear.com/7.x/initials/svg?seed=" + encodeURIComponent(me?.name || me?.email || "U");
      ...
      <div className="card" style={{ display: "flex", gap: 16, alignItems: "center" }}>
        <div style={{ position: "relative", width: 96, height: 96 }}>
          <img
            src={avatarSrc}
            alt="avatar"
            style={{ width: 96, height: 96, borderRadius: "9999px", objectFit: "cover", border: "2px solid #e5e7eb" }}
          />
          <label htmlFor="avatarPick" className="btn" style={{ position: "absolute", bottom: -6, right: -6, fontSize: 12, padding: "6px 10px" }}>
            Change
          </label>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ color: "#666", fontSize: 12 }}>Role: {me.role} • Grade: {me.grade}</div>
        </div>
      </div>
      ...
      {toast && (
        <div
          style={{
            ...
            color: "white", padding: "12px 16px", borderRadius: 12, zIndex: 9999,
