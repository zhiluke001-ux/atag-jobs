...
type AuthCtx = {
  ...
  login:(email:string)=>Promise<User>;   // <-- return User
  logout:()=>Promise<void>;
...
const Ctx = createContext<AuthCtx>({
  ...
  login:async()=>null as any,  // <-- default matches the new return type
  logout:async()=>{},
  refresh:async()=>{}
});
...
export function AuthProvider({ children }:{children:React.ReactNode}){
  ...
  async function login(email:string){
    ...
    const j = await r.json();
    await refresh();
    return (j?.user ?? null) as NonNullable<User>;  // <-- return the user
  }
